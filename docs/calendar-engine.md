...
- **Mechanism**: Each master's current-academic-year analysis is cached **independently** with a versioned key (for example `analyze-events-v2-2026-DAC`). When multiple masters are requested, the system fetches only the uncached ones and merges the results.
