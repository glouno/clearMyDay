...
export default function WeeklyCalendarPreview({ courseGroups, selectedCourses, selectedMasters, autoLoad = false, dateRange }: WeeklyCalendarPreviewProps) {
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      // Preview without creating or updating a persistent subscription token.
      ...
      const icsContent = data.data.icsContent;
      if (typeof icsContent !== 'string') {
        throw new Error('Preview response did not include calendar data');
      }
      ...
      // Expand recurring events within the same current window selected by the user.
      const academicYearStart = dateRange?.start || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000);
      const academicYearEnd = dateRange?.end || new Date(Date.now() + 365 * 24 * 60 * 60 * 1000);
